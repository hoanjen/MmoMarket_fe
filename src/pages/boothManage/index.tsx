...
function CreateBooth({ isRefresh }: { isRefresh?: React.Dispatch<React.SetStateAction<boolean>> }) {
    ...
    if (!fileExtension || !allowedExtensions.includes(fileExtension)) {
      ...
      if (file !== undefined) {
        ...
        if (res?.statusCode == 400) {
          toast.error('Tải ảnh thất bại, ảnh có kích thước quá lớn !', {
            position: 'top-right',
            autoClose: 2000,
    ...
    <div className="bg-white p-8 rounded-md w-full">
      <div className=" flex justify-end ">
        <div className="space-x-8">
          <CreateBooth isRefresh={setIsRefresh} />
        </div>
      </div>
      <div>
        <ProductOwnerTable isRefresh={isRefresh}></ProductOwnerTable>
      </div>
    </div>
  );
}
